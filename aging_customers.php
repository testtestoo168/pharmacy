...
$pageTitle = t('accounting.aging_customers');
require_once 'includes/header.php';
$tid = getTenantId();
$bid = getBranchId();
requirePermission('reports_financial');

$customers = $pdo->prepare("SELECT c.id, c.name, c.phone, c.balance,
...
$customers->execute([$tid,$bid,$tid,$bid,$tid,$bid,$tid,$bid,$tid]);
...
<div class="card"><div class="card-header"><h3><i class="fas fa-users"></i> <?= t('accounting.aging_customers') ?> (<?=count($data)?>)</h3></div>
    <div class="table-responsive"><table>
        <thead><tr><th><?= t('sales.customer') ?></th><th><?= t('phone') ?></th><th>0-30 <?= t('days') ?></th><th>31-60 <?= t('days') ?></th><th>61-90 <?= t('days') ?></th><th>90+ <?= t('days') ?></th><th><?= t('total') ?></th></tr></thead>
        <tbody>
